...
const CustomerTable = () => {
    const { data, isLoading, isError } = useQuery({
        queryKey: ["clients"],
        queryFn: async () => {
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL ? `https://${process.env.NEXT_PUBLIC_BASE_URL}` : 'http://localhost:3000'}/api/users`);
    ...
    if (isError || !data?.clients)
        return (
        <p className="text-red-500 text-center mt-6">Failed to load customers.</p>
        );
    ...
    return (
        ...
        <table className="w-full table-auto text-sm text-gray-200 bg-gray-800">
            ...
            <tr>
                ...
                <th className="p-4">Total Spent (MAD)</th>
            ...
            {customers.map((customer, i) => (
                ...
                <td className="p-4 text-center">{customer.address ?? "Morocco"}</td>
                <td className="p-4 text-center">{customer.totalOrders ?? 0}</td>
                <td className="p-4 text-center">{customer.totalSpent?.toFixed(2) ?? "0.00"}</td>
